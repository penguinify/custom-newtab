...
`);
        ...
        let inputElement = this.pens.getById(`searchbar-input-${this.id}`);

        // Set placeholder color using JS for cross-browser support
        if (inputElement.element instanceof HTMLInputElement) {
            try {
                inputElement.element.style.setProperty('color', this.data.data.inputTextColor || '#FFFFFF');
...
`;
    ...
    search(query: string): void {
        let searchURL = this.data.data.searchEngineURL || "https://www.google.com/search?q=%s";
        let finalURL = searchURL.replace("%s", encodeURIComponent(query));
