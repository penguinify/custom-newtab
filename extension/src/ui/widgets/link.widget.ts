...
export class LinkWidget extends Widget<WidgetConfig<LinkData>> {
	render(): PenArray {
		...
		textDecoration =
			this.data.data.textDecoration?.trim() === ""
				? "underline"
				: this.data.data.textDecoration;
		url =
			this.data.data.url?.trim() === ""
				? "penguinify.github.io"
				: this.data.data.url;
		openInNewTab =
			typeof this.data.data.openInNewTab === "boolean"
				? this.data.data.openInNewTab
				: false;
		...
		const linkElem = this.pens
			.getById(`text-widget-${this.id}`)
			.asPenArray()
			.querySelector("a");
		if (linkElem) {
	...
	static getWidgetOptionsRecord(): WidgetOptionsRecord {
		return {
			...
			openInNewTab: new CheckboxOption(
				"Open in New Tab",
				"Open the link in a new browser tab",
			),
			textDecoration: new TextOption(
				"Text Decoration",
				"CSS text-decoration property (e.g., underline, none)",
			),
		};
	}
}
