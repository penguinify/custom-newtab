...
import { Elements, Pen } from "../framework/penexutils";
import { UserConfig, WidgetOptionsRecord } from "../types";
import { TextOption, ColorOption, CheckboxOption } from "./widgetoptions";
import { SVGDiscreteFilterId, createSVGDiscreteFilter } from "../utils/color";
...
export class TextOptionMixin extends OptionMixin {
    ...
    static getWidgetOptionsRecord(): WidgetOptionsRecord {
        return {
            ...
            fontSmoothing: new CheckboxOption("Font Smoothing", "Enable or disable font smoothing")
        };
    }

    // i get any remove the point of typescript but im WAY to lazy to make this generic properly
    applyTextOptions<T>(pen: Pen<Elements>, config: UserConfig): void {
        // Get default options from the mixin
        const defaultConfig = (this.constructor as typeof TextOptionMixin).defaultOptions();
        ...
        const resolvedFontSize = !merged.fontSize || isNaN(merged.fontSize) || merged.fontSize <= 0 ? "16px" : `${merged.fontSize}px`;
        ...
        pen.element.style.setProperty("font-family", resolvedFontFamily, "important");
        pen.element.style.setProperty("color", resolvedColor, "important");
        pen.element.style.setProperty("font-weight", resolvedFontWeight, "important");
