...
<p class="mb-4 text-white/75">${this.description}</p>
        ...
        `);
        let containerPen = container.getById(`widget-options-${this.id}`);
    ...
    private _createOption(option: SettingOptions): PenArray {
        ...
        switch (option.type) {
            case 'colorpicker':
                this.pens.push(...new ColorPicker(this.pens.getById(`widget-options-${this.id}`), option).render());
                break;
            case 'dropdown':
                this.pens.push(...new DropdownWithSubcomponents(this.pens.getById(`widget-options-${this.id}`), option).render());
                break;
            case 'text':
                this.pens.push(...new TextInput(this.pens.getById(`widget-options-${this.id}`), option).render());
                break;
            case 'file':
                this.pens.push(...new FileInput(this.pens.getById(`widget-options-${this.id}`), option).render());
                break;
            case "checkbox":
                this.pens.push(...new CheckboxInput(this.pens.getById(`widget-options-${this.id}`), option).render());
                break;
            case "button":
                this.pens.push(...new ButtonInput(this.pens.getById(`widget-options-${this.id}`), option).render());
    ...
    hide(): void {
        ...
        this.pens.getById(`widget-options-${this.id}`).element.parentElement.style.display = 'none';
    }

    show(): void {
        this.pens.getById(`widget-options-${this.id}`).element.parentElement.style.display = 'flex';
    }
