...
widget-layer w-3/4 h-1/4 flex flex-row items-center bottom-0 right-0 border-t-2 border-white absolute bg-black p-4 overflow-x-auto overflow-y-hidden">
	...
	private renderWidgets() {
		...
		for (
			let i = WidgetEditorRenderer.WidgetEditorInstances.length - 1;
			i >= 0;
			i--
		) {
...
<button id="widget-layer-editor-left" class="text-3xl rounded-md mb-2 text-white cursor-pointer"><span class="text-sm">right</span><span>➫</span></button>
                ...
                <h4 class="text-lg text-white h-1/2">${widgetInstance.data.WidgetRecordId}</h4>

<div class="flex w-full" id="widget-layer-lock-${widgetInstance.id}">
<span class="text-white">lock widget</span>
</div>
            </div>
        `);
		widgetPen[0].setParent(this.widgetLayerEditor);
		widgetPen[0].element.addEventListener(
			"click",
			this.clickHandler.bind(this, widgetInstance),
		);

		const leftButton = widgetPen.getById(`widget-layer-editor-left`);
		const rightButton = widgetPen.getById(`widget-layer-editor-right`);
		const lockDiv = widgetPen.getById(`widget-layer-lock-${widgetInstance.id}`);
		...
		lockDiv.element.addEventListener("click", (event) => {
			event.stopPropagation();
			this.lockWidgetInstance(event, widgetInstance);
		});
	}

	private lockWidgetInstance(
		event: MouseEvent,
		widgetInstance: WidgetEditorRenderer<any>,
	) {
		widgetInstance.setLocked(!widgetInstance.locked);

		if (widgetInstance.locked) {
			(event.currentTarget as HTMLElement).innerHTML =
				`<span class="text-red-500">🔒 widget locked</span>`;
		} else {
			(event.currentTarget as HTMLElement).innerHTML =
				`<span class="text-white">lock widget</span>`;
		}
	...
	private clickHandler(
		WidgetEditorInstance: WidgetEditorRenderer<any>,
		event: MouseEvent,
	) {
		event.stopPropagation();
