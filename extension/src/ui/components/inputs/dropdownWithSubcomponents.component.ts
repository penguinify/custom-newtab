...
export class DropdownWithSubcomponents implements Component {
	...
	constructor(parent: Pen<Elements>, options: SettingOptions) {
		...
		if (options.type !== "dropdown") {
			throw new Error(
				"Invalid option type for DropdownWithSubcomponents component",
			);
		} else {
	...
	render(): PenArray {
		if (
			this.settings.type !== "dropdown" ||
			!this.settings.options ||
			this.settings.options.length === 0
		) {
			throw new Error(
				"Invalid settings for DropdownWithSubcomponents component",
			);
		}
		...
		const select = container.getById(`${this.id}-select`);
				...
				for (const subOpt of this.settings.subOptions[key]) {
					...
					if (Comp) {
						const compInstance = new Comp(
							container.getById(`${this.id}-subcomponents`),
							subOpt,
						);
