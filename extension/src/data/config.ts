...
export const updateConfigEvent = new CustomEvent(
	"userConfigUpdated",
	updateConfigEventDetails,
);
...
export async function setPathInUserConfig<T>(
	path: string[],
	value: T,
): Promise<void> {
...
export async function getUserConfig(): Promise<UserConfig> {
	...
	if (isUserConfig(storedConfig)) {
		...
		const mergedConfig = mergeUserConfig(
			defaultConfig,
			storedConfig as Partial<UserConfig>,
		);
