...
export class WidgetRegistry {
	...
	static registerWidget<T extends WidgetConfig<Object>>(
		widgetId: string,
		widgetClass: new (data: T) => Widget<T>,
	): void {
		if (DEBUG_LOGGING) {
	...
	static getWidget<T extends WidgetConfig<Object>>(
		widgetId: string,
	): (new (data: T) => Widget<T>) | undefined {
		return WidgetRegistry.registry[widgetId] as
			| (new (
					data: T,
			  ) => Widget<T>)
			| undefined;
