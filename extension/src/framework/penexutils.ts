...
class Components {
	...
	add(...components: Component[]): void {
		if (DEBUG_LOGGING)
			console.debug("[Components] Adding components:", components);
		components.forEach((component) => {
			if (component.render === undefined) return;
			if (DEBUG_LOGGING)
				console.debug("[Components] Rendering component:", component);
...
class Pen<T extends Elements> {
	...
	constructor(tag: string, parent?: T | elementGlobals) {
		if (DEBUG_LOGGING)
			console.debug("[Pen] Creating element:", tag, "with parent:", parent);
	...
	setParent(
		parent: HTMLElement | elementGlobals | Pen<Elements>,
		childNumber?: number,
	): void {
		if (DEBUG_LOGGING)
			console.debug(
		...
		if (parent instanceof HTMLElement) {
			...
			if (childNumber !== undefined && childNumber < parent.children.length) {
				...
				if (DEBUG_LOGGING)
					console.debug("[Pen] Inserted element at childNumber:", childNumber);
			} else {
			...
			if (
				...
				childNumber < parent.element.children.length
			) {
				parent.element.insertBefore(
					this.element,
					parent.element.children[childNumber],
				);
				if (DEBUG_LOGGING)
					console.debug(
						"[Pen] Inserted element at childNumber in Pen:",
						childNumber,
					);
			} else {
				parent.element.appendChild(this.element);
				if (DEBUG_LOGGING)
					console.debug("[Pen] Appended element to Pen parent");
			}
		}
	}
	...
	static fromElement<E extends Elements>(
		element: E,
		parent?: E | elementGlobals,
	): Pen<E> {
		if (DEBUG_LOGGING)
			console.debug(
				"[Pen] Creating from existing element:",
				element,
				"with parent:",
				parent,
			);
		const pen: Pen<E> = new Pen(element.tagName);
		pen.element = element;
...
export class PenArray extends Array<Pen<Elements>> {
	...
	querySelectorAll(selector: string): PenArray {
		...
		const pens = new PenArray(
			...sortedElements.map((el) => Pen.fromElement(el as HTMLElement)),
		);
		if (DEBUG_LOGGING)
			console.debug("[PenArray] querySelectorAll result:", pens);
		return pens;
	}

	querySelector(selector: string): Pen<Elements> | null {
		if (DEBUG_LOGGING) console.debug("[PenArray] querySelector:", selector);
		for (const pen of this) {
			if (pen.element.matches(selector)) {
				if (DEBUG_LOGGING)
					console.debug("[PenArray] querySelector found match:", pen);
				return pen;
			}
			const child = pen.element.querySelector(selector);
			if (child) {
				if (DEBUG_LOGGING)
					console.debug("[PenArray] querySelector found child match:", child);
	...
	static fromHTML(html: HTMLMarkup): PenArray {
		...
		Array.from(element.children).forEach((child) => {
			...
			pens.push(pen);
			if (DEBUG_LOGGING)
				console.debug("[PenArray] fromHTML added direct child:", pen);
		});
		...
		element.querySelectorAll("*").forEach((child) => {
			...
			pens.push(pen);
			if (DEBUG_LOGGING)
				console.debug("[PenArray] fromHTML added nested child:", pen);
		});
...
export class Router {
	...
	private appendRouteToMainApp(route: Route): void {
		if (DEBUG_LOGGING)
			console.debug("[Router] Appending route to main app:", route);
		// reload webpage
		window.document.body.innerHTML = "";

		const layout = route.layout(this.defaultLayout) || this.defaultLayout;
		if (DEBUG_LOGGING) console.debug("[Router] Using layout:", layout);
		setup(this.app, layout, route.components);

		const mainApp = document.getElementById(this.app);
		...
		for (let i = 0; i < route.pens.length; i++) {
			try {
				if (route.pens[i].parent === elementGlobals.mainApp) {
					...
					if (DEBUG_LOGGING)
						console.debug("[Router] Appended pen to mainApp:", route.pens[i]);
				}
			} catch (e) {
				if (DEBUG_LOGGING)
					console.error(
						"[Router] Error appending pen:",
						e,
						"at component",
						route.pens[i],
					);
	...
	async navigateTo(path: string): Promise<void> {
		...
		if (!route) {
			if (DEBUG_LOGGING)
				console.error("[Router] No route with path found:", path);
		...
		if (route instanceof AsyncRoute) {
			if (DEBUG_LOGGING)
				console.debug("[Router] Route is AsyncRoute, rendering fallback");
			const fallBackPens = route.renderFallback();
			route.pens = fallBackPens;
			this.appendRouteToMainApp(route);

			route.pens = await route.pensAsync;
			if (DEBUG_LOGGING)
				console.debug("[Router] AsyncRoute pens loaded:", route.pens);
		}
	...
	reloadCurrentRoute(): void {
		if (DEBUG_LOGGING)
			console.debug("[Router] Reloading current route:", this.path);
		this.navigateTo(this.path);
	}
}
...
export function sanitize(unsafe_string: string): string {
	if (DEBUG_LOGGING)
		console.debug("[sanitize] Sanitizing string:", unsafe_string);
	const map: {
		[key: string]: string;
	} = {
		"&": "&amp;",
	...
	const result = unsafe_string.replace(/[&<>"']/g, (m) => map[m]);
