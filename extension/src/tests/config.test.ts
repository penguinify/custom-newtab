...
import { defaultUserConfig, mergeUserConfig } from "../data/config";
...
describe("UserConfig merging", () => {
	...
	it("deep merges background of type 'color'", () => {
		...
		expect(
			(merged.background as Extract<BackgroundType, { type: "color" }>).hex,
		).toBe("#123456");
	});
	...
	it("deep merges background of type 'image'", () => {
		...
		expect(
			(merged.background as Extract<BackgroundType, { type: "image" }>).url,
		).toBe("img.png");
		expect(
			(merged.background as Extract<BackgroundType, { type: "image" }>).css,
		).toBe("background-size:cover;");
	});
	...
	it("deep merges background of type 'video'", () => {
		...
		expect(
			(merged.background as Extract<BackgroundType, { type: "video" }>)
				.fallbackColor,
		).toBe("#ff00ff");
	});
	...
	it("deep merges background of type 'customcss'", () => {
		...
		expect(
			(merged.background as Extract<BackgroundType, { type: "customcss" }>).css,
		).toBe(".foo { color: red; }");
	});
